...
void Tracking::Track()
{
    ...
    else// 步骤2：跟踪
    {
        ...
        else
        {
            ...
            // 重定位成功，
            //bOK=true表示track成功，mbVO=true表示在MapPoints中有很少的匹配点
            //如果重定位成功那么就跟踪局部地图
            if(bOK && !mbVO)
                bOK = TrackLocalMap();
        }
...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        if(mpInitializer->Initialize(mCurrentFrame, mvIniMatches, Rcw, tcw, mvIniP3D, vbTriangulated))  //这里的输入依次为（当前帧、匹配的特征点、旋转矩阵、平移向量，3d点向量、是否可以三角化）
        {
            // 步骤6：删除那些无法进行三角化的匹配点
